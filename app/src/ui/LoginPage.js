...
const LoginPage = () => {
    ...
    const handleLogin = () => {
        ...
        if (amountOfLogin === "5") {
            setAlertMessage("Nie udało się zalogować. Nieprawidłowy login lub hasło");
            handleClick();
        } else {
            axios
                ...
                .then((res) => {
                    if (res.status === 200) {
                        if (res.data) {
                            ...
                            if (rememberMe) {
                                Cookies.set("AuctionPageSavedLogin", `${login}/${pass}`, {expires: 3});
                            }
                            history.push(`/logged/${res.data._id}`);
                        } else {
                            if (amountOfLogin) {
                                Cookies.set("BlockedLogin", parseInt(amountOfLogin) + 1, {
                                    expires: new Date(new Date().getTime() + parseInt(blockingMultiplier) * 10 * 60 * 1000)
                                });
                                if ((parseInt(amountOfLogin) + 1) === 5) {
                                    Cookies.set("BlockingMultiplier", parseInt(blockingMultiplier) + 1, {expires: 1});
                                }
                            } else {
                        ...
                        setAlertMessage("Nie udało się zalogować. Błąd serwera");
                        handleClick();
                    }
                });
        }
