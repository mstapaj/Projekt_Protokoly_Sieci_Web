...
const AccountManagement = () => {
    ...
    const getAllUsers = () => {
        axios.get("https://localhost:5000/users").then((res) => {
            if (res.status === 200) {
                setUsers(
                    res.data.filter((n) => n._id !== "000000000000000000000000")
                );
            } else {
    ...
    return (
        <div className={"auction-list"}>
            ...
            <div className={"items"}>
                {users.length > 0 &&
                    users.map((n) => (
                        <div key={n._id} className={"space-bet"}>
                            <p
                                className={"chatroom"}
                                onClick={() =>
                                    history.push(
                                        `/logged/${params.id}/user/${n._id}`
                                    )
                                }
                            >
                                {n.login}
                            ...
                            <div className={"buttons"}>
                                <Button
                                    ...
                                    onClick={() => {
                                        history.push(
                                            `/logged/${params.id}/accountManagement/${n._id}/editAccount`
                                        );
                                    }}
                                >
                                    Edytuj użytkownika
